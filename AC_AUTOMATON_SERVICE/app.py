...
try:
    ...
    print(f"ERROR: Could not connect to Neo4j database: {e}", file=sys.stderr)
    # 不直接退出，而是让健康检查反映状态
...
try:
    ...
    print("EntityExtractor 初始化成功。")
except FileNotFoundError as e:
    print(f"ERROR: Entity data file not found: {e}", file=sys.stderr)
    print("Please run 'python scripts/extract_entities_from_neo4j.py' to generate 'all_graph_entities.jsonl'.", file=sys.stderr)
    # sys.exit(1) # 如果实体数据文件不存在，则直接退出
except ValueError as e:
    print(f"ERROR during EntityExtractor initialization: {e}", file=sys.stderr)
    # sys.exit(1)

try:
    intent_classifier = RuleBasedIntentClassifier()
    print("RuleBasedIntentClassifier 初始化成功。")
except Exception as e:
    print(f"ERROR during RuleBasedIntentClassifier initialization: {e}", file=sys.stderr)
...
def health_check():
    """
    AC 自动机服务的健康检查端点。
    检查 Neo4j 连接和核心组件是否已初始化。
    """
    health_status = {"status": "ok", "service": "AC Automaton Service"}
    ...
    if neo4j_driver:
        try:
            ...
            health_status["neo4j_connection"] = f"failed: {e}"
            health_status["status"] = "degraded"
    else:
        health_status["neo4j_connection"] = "not_initialized"
        health_status["status"] = "degraded"

    # 检查 EntityExtractor
    if entity_extractor:
        health_status["entity_extractor"] = "initialized"
    else:
        health_status["entity_extractor"] = "not_initialized"
        health_status["status"] = "degraded"

    # 检查 IntentClassifier
    if intent_classifier:
        health_status["intent_classifier"] = "initialized"
    else:
        health_status["intent_classifier"] = "not_initialized"
        health_status["status"] = "degraded"
    
    return jsonify(health_status), 200 # 返回 200 OK 即使状态是 degraded
...
def generate_recommended_questions(query_text, extracted_entities, predicted_intent):
    ...
    ] if main_entity_name else [] # 确保main_entity_name存在才添加

    # Neo4j 查询会话
    if neo4j_driver is None:
        print("WARN: Neo4j driver not initialized, cannot generate graph-based recommendations.", file=sys.stderr)
        return recommended_questions # 返回空列表或通用问题
    ...
    try:
        ...
        print(f"ERROR: Error during Neo4j query for recommendations: {e}", file=sys.stderr)
        # 如果 Neo4j 查询失败，可以返回通用问题
        recommended_questions.append({
            "questionText": "知识图谱查询失败，请稍后再试。",
            "questionId": "neo4j_error",
            "relevantEntities": [],
            "relevanceScore": 0.1
        })
...
def process_query():
    ...
    if entity_extractor is None:
        return jsonify({"apiResponse": {"code": 500, "message": "EntityExtractor not initialized"}}), 500
    ...
    if intent_classifier is None:
        return jsonify({"apiResponse": {"code": 500, "message": "IntentClassifier not initialized"}}), 500
